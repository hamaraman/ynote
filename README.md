...
applyUrl: "https://..."      # 검증된 직접 신청 페이지 URL (최우선)
officialUrl: "https://..."   # 직접 신청 URL이 없을 때의 공식 공고·신청 포털 URL
applyLinkLabel: "바로 신청하기" # 선택: 버튼 문구
---
...
`applyUrl`에는 실제 접수 화면으로 바로 이동하는 **검증된 URL**만 넣습니다. 직접 접수 화면이 없거나 정책 공고마다 바뀌는 경우에는 `officialUrl`에 공식 포털 또는 공고 URL을 넣고, 버튼 문구를 `공식 신청처로 이동`처럼 명확히 작성합니다. 둘 다 없으면 사이트는 잘못된 외부 링크로 보내지 않고 `신청 방법 확인하기` 버튼을 표시합니다.
