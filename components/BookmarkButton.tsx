...
export default function BookmarkButton({ policy, variant = "default" }: { policy: BookmarkedPolicy; variant?: "default" | "icon" }) {
    ...
    if (!mounted) {
        ...
        return (
            <button className="flex items-center justify-center gap-1.5 px-4 py-3 text-sm border border-gray-200 rounded-2xl bg-gray-50 text-gray-400 dark:border-slate-700 dark:bg-slate-800/50 font-bold">
                <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                </svg>
                저장하기
    ...
    return (
        <button
            onClick={toggle}
            className={`flex items-center justify-center gap-1.5 px-4 py-3 text-sm border rounded-2xl font-bold transition-all ${
                isBookmarked
                    ? "border-rose-200 bg-rose-50 text-rose-600 hover:bg-rose-100 dark:border-rose-950 dark:bg-rose-950/30 dark:text-rose-400 dark:hover:bg-rose-950/50"
                    : "border-gray-300 bg-white text-gray-750 hover:bg-gray-50 hover:text-gray-900 dark:border-slate-700 dark:bg-slate-800 dark:text-gray-300 dark:hover:bg-slate-700 dark:hover:text-white"
            }`}
        >
            <svg
                className="w-4 h-4 transition-transform active:scale-125"
